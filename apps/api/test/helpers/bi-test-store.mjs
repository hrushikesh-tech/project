...
export function createBiHarness(options = {}) {
  ...
  const ensureEmployee = (overrides = {}) => {
    ...
    const record = {
      id: nextId("employee"),
      employeeNumber: overrides.employeeNumber ?? `EMP-${sequence}`,
      ...
      jobTitle: overrides.jobTitle ?? "Analyst",
  ...
  const insertProject = (overrides = {}) => {
    const managerId = overrides.managerId ?? state.users[0]?.id ?? ensureUser().id;
    const record = {
      id: nextId("project"),
